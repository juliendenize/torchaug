...
class TestBatchLabels:
    ...
    def batch_labels(self):
        ...
        idx_sample = [(0, 1), (1, 2)]
        return BatchLabels(data, idx_sample=idx_sample)
    ...
    def test_cat(self, batch_labels):
        batch_labels_1 = BatchLabels(
            torch.tensor([[[3, 3, 0], [0, 3, 3]], [[4, 4, 0], [0, 4, 4]]]), idx_sample=[(0, 2), (2, 2)]
        )
        batch_labels_2 = BatchLabels(torch.tensor([[[5, 5, 0], [0, 5, 5]]]), idx_sample=[(0, 1)])
        ...
        expected_idx_sample = [(0, 1), (1, 2), (2, 4), (4, 4), (4, 5)]

        assert torch.all(torch.eq(result.data, expected_data))
        assert result.idx_sample == expected_idx_sample
    ...
    def test_update_chunk_(self, batch_labels):
        ...
        chunk = BatchLabels(torch.tensor([[[6, 6, 0], [0, 6, 6]]]), idx_sample=[(0, 1)])
