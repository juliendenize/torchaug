...
def check_functional(functional, input, *args, check_scripted_smoke=True, **kwargs):
    ...
    if (
        isinstance(input, tv_tensors.BoundingBoxes)
...
def check_functional_kernel_signature_match(functional, *, kernel, input_type):
    ...
    for functional_param, kernel_param in zip(functional_params, kernel_params):
        ...
        if issubclass(input_type, PIL.Image.Image):
            ...
            functional_param._annotation = (
                kernel_param._annotation
...
def _make_transform_sample(transform, *, image_or_video, adapter):
    ...
    input = dict(
        ...
        bounding_boxes_xyxy=make_bounding_boxes(
            size, format=tv_tensors.BoundingBoxFormat.XYXY, device=device
        ),
        bounding_boxes_xywh=make_bounding_boxes(
            size, format=tv_tensors.BoundingBoxFormat.XYWH, device=device
        ),
        bounding_boxes_cxcywh=make_bounding_boxes(
            size, format=tv_tensors.BoundingBoxFormat.CXCYWH, device=device
        ),
...
def _check_transform_sample_input_smoke(transform, input, *, adapter):
    ...
    if not check_type(
        input, (is_pure_tensor, PIL.Image.Image, tv_tensors.Image, tv_tensors.Video)
    ):
        return
    ...
    for container_type in [dict, list, tuple]:
        ...
        for output_item, input_item, should_be_transformed in zip(
            output_flat,
            input_flat,
            transforms.Transform()._needs_transform_list(input_flat),
        ):
...
def check_transform(
    transform, input, check_v1_compatibility=True, check_sample_input=True
):
    ...
    if isinstance(input, tv_tensors.BoundingBoxes) and not isinstance(
    ...
    if check_v1_compatibility:
        _check_transform_v1_compatibility(
            transform, input, **_to_tolerances(check_v1_compatibility)
        )
...
def reference_affine_bounding_boxes_helper(
    bounding_boxes, *, affine_matrix, new_canvas_size=None, clamp=True
):
    ...
    return tv_tensors.BoundingBoxes(
        torch.cat(
            [affine_bounding_boxes(b) for b in bounding_boxes.reshape(-1, 4).unbind()],
            dim=0,
...
class TestResize:
    ...
    @pytest.mark.parametrize(
        "make_mask", [make_segmentation_mask, make_detection_masks]
    )
    ...
    def test_bounding_boxes_correctness(self, format, size, use_max_size, fn):
        ...
        expected = self._reference_resize_bounding_boxes(
            bounding_boxes, size=size, **max_size_kwarg
        )
    ...
    @pytest.mark.parametrize(
        "interpolation", set(transforms.InterpolationMode) - set(INTERPOLATION_MODES)
    )
    @pytest.mark.parametrize(
        "make_input",
    ...
    def test_pil_interpolation_compat_smoke(self, interpolation, make_input):
        ...
        with (
            ...
            # This error is triggered in PyTorch core
                ...
                NotImplementedError, match=f"got {interpolation.value.lower()}"
            )
        ):
            F.resize(
                input,
    ...
    def test_functional_pil_antialias_warning(self):
        with pytest.warns(
            UserWarning, match="Anti-alias option is always applied for PIL Image input"
        ):
            F.resize(
                make_image_pil(self.INPUT_SIZE),
                size=self.OUTPUT_SIZES[0],
                antialias=False,
            )
    ...
    def test_interpolation_int(self, interpolation, make_input):
        ...
        if (
            ...
            and interpolation is transforms.InterpolationMode.NEAREST_EXACT
        ):
            return
...
class TestAffine:
    ...
    def test_functional_image_correctness(
        self, angle, translate, scale, shear, center, interpolation, fill
    ):
        image = make_image(dtype=torch.uint8, device="cpu")
    ...
    def _reference_affine_bounding_boxes(
        self, bounding_boxes, *, angle, translate, scale, shear, center
    ):
        if center is None:
    ...
    def test_functional_bounding_boxes_correctness(
        self, format, angle, translate, scale, shear, center
    ):
        bounding_boxes = make_bounding_boxes(format=format)
    ...
    def test_transform_bounding_boxes_correctness(self, format, center, seed):
        ...
        transform = transforms.RandomAffine(
            **self._CORRECTNESS_TRANSFORM_AFFINE_RANGES, center=center
        )
...
class TestRotate:
    ...
    @pytest.mark.parametrize("fill", CORRECTNESS_FILLS)
    def test_functional_image_correctness(
        self, angle, center, interpolation, expand, fill
    ):
        image = make_image(dtype=torch.uint8, device="cpu")
    ...
    def _reference_rotate_bounding_boxes(
        self, bounding_boxes, *, angle, expand, center
    ):
        if center is None:
            center = [s * 0.5 for s in bounding_boxes.canvas_size[::-1]]
        cx, cy = center
    ...
    def test_functional_bounding_boxes_correctness(self, format, angle, expand, center):
        ...
        expected = self._reference_rotate_bounding_boxes(
            bounding_boxes, angle=angle, expand=expand, center=center
        )
    ...
    def test_transform_bounding_boxes_correctness(self, format, expand, center, seed):
        ...
        transform = transforms.RandomRotation(
            **self._CORRECTNESS_TRANSFORM_AFFINE_RANGES, expand=expand, center=center
        )
        ...
        expected = self._reference_rotate_bounding_boxes(
            bounding_boxes, **params, expand=expand, center=center
        )
...
class TestContainerTransforms:
    ...
    def test_random_apply(self, p, sequence_type):
        ...
        output = check_transform(
            transform,
            input,
            check_v1_compatibility=issubclass(sequence_type, nn.ModuleList),
        )
...
class TestToDtype:
    ...
    def test_others_catch_all_and_none(self, make_input):
        ...
        out = transforms.ToDtype(dtype={tv_tensors.Mask: torch.int64, "others": None})(
            sample
        )
        assert out["inpt"].dtype == inpt_dtype
        assert out["bbox"].dtype == bbox_dtype
    ...
    def test_errors_warnings(self, make_input):
        ...
        with pytest.warns(
            ...
            transforms.ToDtype(
                dtype={torch.Tensor: torch.float32, tv_tensors.Image: torch.float32}
            )
...
class TestAdjustBrightness:
    ...
    @pytest.mark.parametrize(
        "make_input", [make_image_tensor, make_image_pil, make_image, make_video]
    )
    def test_functional(self, make_input):
        check_functional(
            F.adjust_brightness,
    ...
    def test_image_correctness(self, brightness_factor):
        ...
        expected = F.to_image(
            F.adjust_brightness(
                F.to_pil_image(image), brightness_factor=brightness_factor
            )
        )
...
class TestRegisterKernel:
    ...
    def test_errors(self):
        ...
        with pytest.raises(
            ValueError, match="Kernels can only be registered on functionals"
        ):
            F.register_kernel(tv_tensors.Image, F.resize)

        with pytest.raises(
            ValueError, match="Kernels can only be registered for subclasses"
        ):
            F.register_kernel(F.resize, object)

        with pytest.raises(
            ValueError, match="cannot be registered for the builtin tv_tensor classes"
        ):
        ...
        with pytest.raises(
            ValueError, match="already has a kernel registered for type"
        ):
            F.register_kernel(F.resize, CustomTVTensor)(resize_custom_tv_tensor)
...
class TestGetKernel:
    ...
    def test_exact_match(self):
        ...
        for input_type, kernel in self.KERNELS.items():
            _register_kernel_internal(
                resize_with_pure_kernels, input_type, tv_tensor_wrapper=False
            )(kernel)
    ...
    def test_builtin_tv_tensor_subclass(self):
        ...
        for custom_tv_tensor_subclass in [
            ...
            _register_kernel_internal(
                ...
                builtin_tv_tensor_class,
                tv_tensor_wrapper=False,
            )(builtin_tv_tensor_kernel)

            assert (
                _get_kernel(resize_with_pure_kernels, custom_tv_tensor_subclass)
                is builtin_tv_tensor_kernel
            )
    ...
    def test_tv_tensor_subclass(self):
        ...
        _register_kernel_internal(F.resize, MyTVTensor, tv_tensor_wrapper=False)(
            resize_my_tv_tensor
        )
...
class TestElastic:
    ...
    def test_transform(self, make_input, size, device):
        ...
        check_v1_compatibility = (
            False if sys.platform == "darwin" else dict(rtol=0, atol=1)
        )
    ...
    def test_correctness(self):
        ...
        for input_value, out_value in zip(input.values(), out.values()):
            if isinstance(input_value, tv_tensors.TVTensor):
                assert isinstance(out_value, torch.Tensor) and not isinstance(
                    out_value, tv_tensors.TVTensor
                )
            else:
...
class TestCrop:
    ...
    def _reference_crop_bounding_boxes(
        self, bounding_boxes, *, top, left, height, width
    ):
        affine_matrix = np.array(
            [
                [1, 0, -left],
    ...
    def test_transform_bounding_boxes_correctness(
        self, output_size, format, dtype, device, seed
    ):
...
class TestErase:
    ...
    def test_kernel_video(self):
        check_kernel(
            F.erase_video, make_video(self.INPUT_SIZE), **self.FUNCTIONAL_KWARGS
        )
    ...
    def test_transform(self, make_input, device):
        ...
        with pytest.warns(
            UserWarning, match="currently passing through inputs of type"
        ):
            check_transform(
...
class TestGaussianBlur:
    ...
    def test_transform(self, make_input, device, sigma):
        check_transform(
            transforms.GaussianBlur(kernel_size=3, sigma=sigma),
            make_input(device=device),
        )
    ...
    def test_functional_image_correctness(
        ...
        expected = (
            torch.tensor(
                self.REFERENCE_GAUSSIAN_BLUR_IMAGE_RESULTS[reference_results_key]
            )
...
class TestAutoAugmentTransforms:
    ...
    def _reference_shear_translate(
    ...
    def test_correctness_shear_translate(
        self, transform_id, magnitude, interpolation, fill, input_type
    ):
    ...
    def _sample_input_adapter(self, transform, input, device):
        ...
        for key, value in input.items():
            if isinstance(value, (tv_tensors.BoundingBoxes, tv_tensors.Mask)):
                ...
                value,
                (tv_tensors.Image, tv_tensors.Video, is_pure_tensor, PIL.Image.Image),
            ):
    ...
    def test_transform_smoke(self, transform, make_input, dtype, device):
        if make_input is make_image_pil and not (
            dtype is torch.uint8 and device == "cpu"
        ):
            pytest.skip(
...
class TestConvertBoundingBoxFormat:
    old_new_formats = list(
        itertools.permutations(iter(tv_tensors.BoundingBoxFormat), 2)
    )
    ...
    def test_strings(self, old_format, new_format):
        ...
        input = tv_tensors.BoundingBoxes(
            torch.tensor([[10, 10, 20, 20]]), format=old_format, canvas_size=(50, 50)
        )
    ...
    def test_correctness(self, old_format, new_format, dtype, device, fn_type):
        bounding_boxes = make_bounding_boxes(
            format=old_format, dtype=dtype, device=device
        )
        ...
        actual = fn(bounding_boxes)
        expected = self._reference_convert_bounding_box_format(
            bounding_boxes, new_format
        )
    ...
    def test_errors(self):
        ...
        with pytest.raises(ValueError, match="`old_format` has to be passed"):
            F.convert_bounding_box_format(
                input_pure_tensor, new_format=input_tv_tensor.format
            )

        with pytest.raises(ValueError, match="`old_format` must not be passed"):
            F.convert_bounding_box_format(
                input_tv_tensor,
                old_format=input_tv_tensor.format,
                new_format=input_tv_tensor.format,
            )
...
class TestResizedCrop:
    ...
    def _reference_resized_crop_bounding_boxes(
        self, bounding_boxes, *, top, left, height, width, size
    ):
...
class TestCenterCrop:
    ...
    def test_image_correctness(self, output_size, fn):
        ...
        expected = F.to_image(
            F.center_crop(F.to_pil_image(image), output_size=output_size)
        )
...
class TestPerspective:
    ...
    def _reference_perspective_bounding_boxes(
        self, bounding_boxes, *, startpoints, endpoints
    ):
        format = bounding_boxes.format
        canvas_size = bounding_boxes.canvas_size
        dtype = bounding_boxes.dtype
    ...
    def test_correctness_perspective_bounding_boxes(
        self, startpoints, endpoints, format, dtype, device
    ):
        bounding_boxes = make_bounding_boxes(format=format, dtype=dtype, device=device)
...
class TestUniformTemporalSubsample:
    ...
    def test_video_correctness(self, num_samples, dtype, device, fn):
        ...
        expected = self._reference_uniform_temporal_subsample_video(
            video, num_samples=num_samples
        )
...
class TestNormalize:
    ...
    def test_kernel_image(self, mean, std, device):
        check_kernel(
            F.normalize_image,
            make_image(dtype=torch.float32, device=device),
            mean=self.MEAN,
            std=self.STD,
        )
    ...
    def _sample_input_adapter(self, transform, input, device):
        ...
        for key, value in input.items():
            if isinstance(value, PIL.Image.Image):
                ...
                value, (is_pure_tensor, tv_tensors.Image, tv_tensors.Video)
            ):
    ...
    def test_errors(self):
        ...
        for format_, canvas_size_ in [
            ...
            with pytest.raises(
                ...
                F.clamp_bounding_boxes(
                    input_tv_tensor, format=format_, canvas_size=canvas_size_
                )
...
class TestInvert:
    ...
    @pytest.mark.parametrize(
        "make_input", [make_image_tensor, make_image, make_image_pil, make_video]
    )
    def test_functional(self, make_input):
...
class TestAdjustSharpness:
    ...
    def test_correctness_image(self, sharpness_factor, fn):
        ...
        expected = F.to_image(
            F.adjust_sharpness(F.to_pil_image(image), sharpness_factor=sharpness_factor)
        )
...
class TestAdjustContrast:
    ...
    def test_correctness_image(self, contrast_factor):
        ...
        expected = F.to_image(
            F.adjust_contrast(F.to_pil_image(image), contrast_factor=contrast_factor)
        )
...
class TestAdjustGamma:
    ...
    def test_correctness_image(self, gamma, gain):
        ...
        expected = F.to_image(
            F.adjust_gamma(F.to_pil_image(image), gamma=gamma, gain=gain)
        )
...
class TestAdjustHue:
    ...
    def test_correctness_image(self, hue_factor):
        ...
        expected = F.to_image(
            F.adjust_hue(F.to_pil_image(image), hue_factor=hue_factor)
        )
...
class TestAdjustSaturation:
    ...
    def test_correctness_image(self, saturation_factor):
        ...
        expected = F.to_image(
            F.adjust_saturation(
                F.to_pil_image(image), saturation_factor=saturation_factor
            )
        )
...
class TestFiveTenCrop:
    ...
    def test_correctness_image_ten_crop(self, fn_or_class, vertical_flip):
        ...
        expected = F.ten_crop(
            F.to_pil_image(image), size=self.OUTPUT_SIZE, vertical_flip=vertical_flip
        )
...
class TestRgbToGrayscale:
    ...
    @pytest.mark.parametrize(
        "make_input", [make_image_tensor, make_image_pil, make_image]
    )
    def test_functional(self, make_input):
    ...
    def test_image_correctness(self, num_output_channels, color_space, fn):
        ...
        expected = F.to_image(
            F.rgb_to_grayscale(
                F.to_pil_image(image), num_output_channels=num_output_channels
            )
        )
    ...
    def test_random_transform_correctness(self, num_input_channels):
        ...
        expected = F.to_image(
            F.rgb_to_grayscale(
                F.to_pil_image(image), num_output_channels=num_input_channels
            )
        )
...
class TestLinearTransform:
    ...
    def test_transform_error(self):
        ...
        transform = transforms.LinearTransformation(
            *self._make_matrix_and_vector(image)
        )
...
class TestToImage:
    @pytest.mark.parametrize(
        "make_input", [make_image_tensor, make_image_pil, make_image, make_image_numpy]
    )
    @pytest.mark.parametrize(
    ...
    def test_functional_error(self):
        with pytest.raises(
            TypeError,
            match="Input can either be a pure Tensor, a numpy array, or a PIL image",
        ):
            F.to_image(object())
...
class TestToPILImage:
    @pytest.mark.parametrize(
        "make_input", [make_image_tensor, make_image, make_image_numpy]
    )
    @pytest.mark.parametrize("color_space", ["RGB", "GRAY"])
    @pytest.mark.parametrize(
        "fn", [F.to_pil_image, transform_cls_to_functional(transforms.ToPILImage)]
    )
...
class TestPILToTensor:
    @pytest.mark.parametrize("color_space", ["RGB", "GRAY"])
    @pytest.mark.parametrize(
        "fn", [F.pil_to_tensor, transform_cls_to_functional(transforms.PILToTensor)]
    )
    def test_functional_and_transform(self, color_space, fn):
        input = make_image_pil(color_space=color_space)
        output = fn(input)

        assert isinstance(output, torch.Tensor) and not isinstance(
            output, tv_tensors.TVTensor
        )
...
def test_pure_tensor_heuristic(make_inputs):
    ...
    (
        first_pure_tensor_input,
        other_pure_tensor_inputs,
        other_inputs,
    ) = split_on_pure_tensor(flat_inputs)

    transform = CopyCloneTransform()
    transformed_sample = transform(flat_inputs)

    (
        first_pure_tensor_output,
        other_pure_tensor_outputs,
        other_outputs,
    ) = split_on_pure_tensor(transformed_sample)

    if first_pure_tensor_input is not None:
        if other_inputs:
            assert not transform.was_applied(
                first_pure_tensor_output, first_pure_tensor_input
            )
        else:
            assert transform.was_applied(
                first_pure_tensor_output, first_pure_tensor_input
            )
...
class TestRandomShortestSize:
    ...
    def test__get_params(self, min_size, max_size):
        ...
        transform = transforms.RandomShortestSize(
            min_size=min_size, max_size=max_size, antialias=True
        )
...
def test_classification_preset(image_type, label_type, dataset_return_type, to_tensor):
    ...
    image = tv_tensors.Image(
        torch.randint(0, 256, size=(1, 3, 250, 250), dtype=torch.uint8)
    )
...
@pytest.mark.parametrize("image_type", (PIL.Image, torch.Tensor, tv_tensors.Image))
@pytest.mark.parametrize(
    "data_augmentation", ("hflip", "lsj", "multiscale", "ssd", "ssdlite")
)
@pytest.mark.parametrize("to_tensor", (transforms.ToTensor, transforms.ToImage))
...
def test_detection_preset(image_type, data_augmentation, to_tensor, sanitize):
    ...
    if data_augmentation == "hflip":
        ...
        t = [
            ...
            transforms.RandomZoomOut(
                fill={"others": (123.0, 117.0, 104.0), tv_tensors.Mask: 0}, p=1
            ),
    ...
    masks = tv_tensors.Mask(
        torch.randint(0, 2, size=(num_boxes, H, W), dtype=torch.uint8)
    )
    ...
    if (
        isinstance(to_tensor, transforms.ToTensor)
        and image_type is not tv_tensors.Image
    ):
