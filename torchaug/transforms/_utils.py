...
def _parse_labels_getter(
    ...
    if labels_getter == "default":
        ...
        raise ValueError(
            f"labels_getter should either be 'default', a callable, or None, but got {labels_getter}."
        )
...
def query_size(flat_inputs: List[Any]) -> Tuple[int, int]:
    ...
    if not sizes:
        ...
        raise ValueError(
            f"Found multiple HxW dimensions in the sample: {sequence_to_str(sorted(sizes))}"
        )
    h, w = sizes.pop()
    return h, w
...
def query_chw(flat_inputs: List[Any]) -> Tuple[int, int, int]:
    ...
    if not chws:
        ...
        raise ValueError(
            f"Found multiple CxHxW dimensions in the sample: {sequence_to_str(sorted(chws))}"
        )
    c, h, w = chws.pop()
    return c, h, w
